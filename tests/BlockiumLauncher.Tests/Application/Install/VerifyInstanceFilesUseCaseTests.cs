...
public sealed class VerifyInstanceFilesUseCaseTests
{
    ...
    [Fact]
    public async Task ExecuteAsync_ReturnsIssue_WhenRootDirectoryIsMissing()
    {
        ...
        Assert.Equal(FileVerificationIssueKind.RootDirectoryMissing, Result.Value.Issues[0].Kind);
    }

    [Fact]
    public async Task ExecuteAsync_ReturnsIssue_WhenMinecraftDirectoryIsMissing()
    {
        ...
        try
        {
            ...
            Assert.True(Result.IsSuccess);
            Assert.False(Result.Value.IsValid);
            Assert.Contains(Result.Value.Issues, x => x.Kind == FileVerificationIssueKind.MinecraftDirectoryMissing);
        }
        finally
        {
    ...
    [Fact]
    public async Task ExecuteAsync_ReturnsIssue_WhenBlockiumDirectoryIsMissing()
    {
        var RootPath = CreateDirectory();
        ...
        try
        {
            ...
            Assert.True(Result.IsSuccess);
            Assert.False(Result.Value.IsValid);
            Assert.Contains(Result.Value.Issues, x => x.Kind == FileVerificationIssueKind.BlockiumDirectoryMissing);
        }
        finally
        {
            DeleteDirectoryIfExists(RootPath);
    ...
    [Fact]
    public async Task ExecuteAsync_ReturnsValid_WhenRequiredDirectoriesExist()
    {
        var RootPath = CreateDirectory();
        Directory.CreateDirectory(Path.Combine(RootPath, ".minecraft"));
        Directory.CreateDirectory(Path.Combine(RootPath, ".blockium"));
