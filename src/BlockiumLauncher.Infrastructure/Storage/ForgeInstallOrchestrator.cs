...
public sealed class ForgeInstallOrchestrator : IForgeInstallOrchestrator
{
            ...
            if (Plan.DownloadRuntime)
            {
                switch (Plan.LoaderType)
                {
                    ...
                    case LoaderType.Forge:
                        ...
                        await DownloadVanillaRuntimeAsync(Plan, RootPath, Context, CancellationToken).ConfigureAwait(false);
                        await DownloadForgeRuntimeAsync(Plan, RootPath, Context, CancellationToken).ConfigureAwait(false);
                        break;
        ...
        CancellationToken CancellationToken)
    {
        ...
        await WriteRuntimeMetadataAsync(RuntimeMetadataPath, RuntimeMetadata, CancellationToken).ConfigureAwait(false);
            ...
            private async Task DownloadForgeRuntimeAsync(
        InstallPlan Plan,
        string RootPath,
        OperationContext Context,
        CancellationToken CancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Could not resolve Java for Forge installer.");
        }

        var JavaPath = JavaResolveResult.Value;
        var ForgeRoot = LauncherPaths.GetSharedLoaderDirectory(LoaderType.Forge, Plan.GameVersion, Plan.LoaderVersion!);
        var InstallerUrl = $"https://maven.minecraftforge.net/net/minecraftforge/forge/{Plan.GameVersion}-{Plan.LoaderVersion}/forge-{Plan.GameVersion}-{Plan.LoaderVersion}-installer.jar";
        var InstallerPath = Path.Combine(ForgeRoot, $"forge-{Plan.LoaderVersion}-installer.jar");
        ...
        var MinecraftRoot = Path.Combine(ForgeRoot, "runtime");
...
""";
        ...
        Logger.Info(Context, nameof(ForgeInstallOrchestrator), "ForgeInstallerStart", "Starting Forge installer.", new
        {
            JavaPath,
            InstallerPath,
            WorkingDirectory = MinecraftRoot,
        ...
        {
            Logger.Error(Context, nameof(ForgeInstallOrchestrator), "ForgeInstallerStartFailed", "Failed to start Forge installer process.", new
            {
                JavaPath,
                InstallerPath,
                WorkingDirectory = MinecraftRoot,
        ...
        Logger.Info(Context, nameof(ForgeInstallOrchestrator), "ForgeInstallerFinished", "Forge installer process finished.", new
        {
        ...
        if (Process.ExitCode != 0)
        {
            throw new InvalidOperationException(
                "Forge installer failed." + Environment.NewLine +
        ...
        await BuildForgeRuntimeMetadataAsync(Plan, RootPath, MinecraftRoot, Context, CancellationToken).ConfigureAwait(false);
    }

            private async Task BuildForgeRuntimeMetadataAsync(
        InstallPlan Plan,
        string RootPath,
        string ForgeRuntimeRoot,
        OperationContext Context,
        CancellationToken CancellationToken)
    {
        var RuntimeMetadataPath = Path.Combine(RootPath, ".blockium", "runtime.json");
        ...
        var VersionsRoot = Path.Combine(ForgeRuntimeRoot, "versions");
        if (!Directory.Exists(VersionsRoot))
        {
            throw new InvalidOperationException("Forge installer did not create a versions directory.");
        }
        ...
        var VersionJsonPath = CandidateJsonFiles.FirstOrDefault(PathValue =>
            Path.GetFileNameWithoutExtension(PathValue).Contains("forge", StringComparison.OrdinalIgnoreCase) ||
        ...
        {
            throw new InvalidOperationException("Could not find installed Forge version json.");
        }

        Logger.Info(Context, nameof(ForgeInstallOrchestrator), "ForgeMetadataSourceResolved", "Resolved Forge metadata source files.", new
        {
            RuntimeMetadataPath,
            ForgeRuntimeRoot,
            VersionsRoot,
        ...
        var LibrariesRoot = Path.Combine(ForgeRuntimeRoot, "libraries");
        ...
        RuntimeMetadata.LoaderType = LoaderType.Forge.ToString();
        RuntimeMetadata.LoaderVersion = Plan.LoaderVersion ?? string.Empty;
        RuntimeMetadata.LoaderProfileJsonPath = Path.Combine(ForgeRuntimeRoot, "forge-version.json");

        var PersistedVersionJsonPath = Path.Combine(ForgeRuntimeRoot, "forge-version.json");
        ...
        CancellationToken CancellationToken)
    {
        ...
        await Parallel.ForEachAsync(
            ...
            async (Item, Ct) =>
            {
                ...
                if (Current % 25 == 0)
                {
                    Logger.Info(Context, nameof(ForgeInstallOrchestrator), "LibrariesProgress", "Library download progress.", new
                    {
        ...
        CancellationToken CancellationToken)
    {
        ...
        await Parallel.ForEachAsync(
            ...
            async (Hash, Ct) =>
            {
                ...
                if (Current % 100 == 0)
                {
                    Logger.Info(Context, nameof(ForgeInstallOrchestrator), "AssetsProgress", "Asset download progress.", new
                    {
